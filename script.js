...
var Bayes = (function (Bayes) {
  ...
  var increment = function (key) {
    ...
    if (!count) count = 0;
    //console.log(key + " " + (parseInt(count) + 1));
  ...
  Bayes.guess = function (text) {
    ...
    console.log("doc counts", docCounts);
    ...
    for (var j = 0; j < labels.length; j++) {
      ...
      for (var i = 0; i < length; i++) {
        ...
        if (_stemTotalCount === 0) {
          ...
          console.log(
            `count of ${word} in lab: ${label} is: ${stemLabelCount(
              word,
              label
            )}`
          );
...
document.querySelector("#test_button").addEventListener("click", go);
...
Bayes.train(
  "La Junta Directiva del Consejo Nacional Electoral (CNE) efectuará hoy una sesión extraordinaria para definir la fecha de las elecciones presidenciales, después de que Nicolás Maduro fuera juramentado ayer Viernes presidente de la República por la Asamblea Nacional.",
  "spanish"
);
Bayes.train(
